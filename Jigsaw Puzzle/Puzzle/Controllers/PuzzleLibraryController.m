...
- (void)viewDidLoad
{
    ...
    thumbs = [[[NSArray alloc] initWithArray:[self imagesForPuzzle]] retain];
    paths = [[[NSArray alloc] initWithArray:[self pathsForImages]] retain];
    contents = [[[NSArray alloc] initWithArray:[self joinData]] retain];
...
- (NSArray*)joinData {
    ...
    NSMutableArray *tempArray = [[[NSMutableArray alloc] initWithCapacity:paths.count] retain];
    for (int i=0; i<paths.count; i++) {
        
        NSArray *objects = [[NSArray arrayWithObjects:[paths objectAtIndex:i], [thumbs objectAtIndex:i], nil] retain];
        NSArray *keys = [[NSArray arrayWithObjects:@"Path", @"Thumb", nil] retain];
        NSDictionary *dict = [[[NSDictionary alloc] initWithObjects:objects forKeys:keys] retain];
...
- (NSArray*)imagesForPuzzle {
    ...
    NSArray *dirContents = [[[NSBundle mainBundle] pathsForResourcesOfType:nil inDirectory:nil] retain];
    NSMutableArray *tempArray = [[[NSMutableArray alloc] initWithCapacity:dirContents.count] retain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (cell == nil) {
            ...
            cell = [[[PhotoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier] retain];
            ...
            UIView *v = [[[UIView alloc] init] retain];
            v.backgroundColor = YELLOW;
            cell.selectedBackgroundView = v;
        }
        
        //NSString *path = [content objectAtIndex:indexPath.row];
        cell.photo.image = [[[contents objectAtIndex:indexPath.row] objectForKey:@"Thumb"] retain];
        return cell;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        NSString *path = [[[contents objectAtIndex:indexPath.row] objectForKey:@"Path"] retain];
