...
- (void)viewDidLoad
{
    ...
    bannerAd = [[BannerViewAd alloc] initWithViewController:self];
    [bannerAd hideAd];
    [FullScreenAd sharedFullScreenAdWithViewController:self];
    
    CGRect screenRect = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
    directions_numbers = [[[NSArray alloc] init] retain];
    directions_positions = [[[NSArray alloc] init] retain];
    ...
    imageViewLattice = [[[UIImageView alloc] initWithImage:image] retain];
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    ...
    //Add the menu
    menu = [[[MenuController alloc] init] retain];
    menu.delegate = self;
    menu.duringGame = NO;
    ...
    {
        menuOrigin = self.bannerAd.frame.size.height;
        menuHeight = self.view.frame.size.height - menuOrigin;
    }
    else
    {
        menuOrigin = menu.view.frame.origin.y;
        menuHeight = menu.view.frame.size.height;
    }
    CGRect menuRect = CGRectMake(menu.view.frame.origin.x, menuOrigin, menu.view.frame.size.width, menuHeight);
    menu.view.frame = menuRect;
    
    //Add the puzzleCompletedController
    NSString *nibName = @"PuzzleCompletedController";
    if( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone )
        nibName = [nibName stringByAppendingFormat:@"_iPhone"];
...
- (void)showCompleteImage
{
    ...
    [FullScreenAd clearFullScreenAd];
    [FullScreenAd sharedFullScreenAdWithViewController:self];
...
- (void)loadPuzzle:(Puzzle*)puzzleDB_ {
    ...
    NSLog(@"Loading puzzle db");

    puzzleDB = puzzleDB_;
    NSLog(@"new puzzle db i s %@",puzzleDB);
            NSLog(@"Loading puzzle db done");
    loadingFailed = NO;
    ...
    if (puzzleDB!=nil) {
        ...
        image = [[UIImage imageWithData:puzzleDB.image.data] retain];
        groups = [[[NSMutableArray alloc] initWithCapacity:NumberSquare/2] retain];
        elapsedTime = [puzzleDB.elapsedTime floatValue];
        percentageLabel.text = [[NSString stringWithFormat:@"%.0d %%", [puzzleDB.percentage intValue]] retain];
...
- (void)puzzlechanged:(NSNotification *)saveNotiNSLfication
{
    NSSet *deletedObjects = [saveNotiNSLfication.userInfo objectForKey:NSDeletedObjectsKey];
    for(NSManagedObject *obj in deletedObjects){
        NSLog(@"Deleteed obj is %@",obj);
//        NSDictionary *changes = [obj changedValues];
        // now process the changes as you need
        
    }
    NSLog(@"Data changed from somethere");
}
...
- (void)puzzleSaved:(NSNotification *)saveNotification {
    ...
    if (![NSThread isMainThread]) {
        ...
        NSLog(@"Puuzle db - found");
     
        puzzleDB = [self lastSavedPuzzle];
        NSLog(@"puzzle db is %@",puzzleDB);
        NSLog(@"logging done");
...
- (void)allPiecesLoaded {
    ...
    if (debugging) {
        ...
        if (loadingGame) {
            ...
            [self shuffleAngles];
            [self refreshPositions];
            [self organizeDrawerWithOrientation:(UIImageOrientation)self.interfaceOrientation];
...
- (void)prepareForNewPuzzle {
    ...
    directions_numbers = [[NSArray arrayWithArray:[self directionsUpdated_numbers]] retain];
    directions_positions = [[NSArray arrayWithArray:[self directionsUpdated_positions]] retain];
    [self computePieceSize];
    [self createLattice];
    drawerFirstPoint = CGPointMake(-4, 5);
    ...
    puzzleOperation = [[[CreatePuzzleOperation alloc] init] retain];
...
- (void)createPieces {
    ...
    if (loadingGame) {
        ...
        imageView.image = image;
        array = [[[NSMutableArray alloc] initWithArray:[self splitImage:image partSize:partSize]] retain];
    ...
    if (loadingGame) {
        ...
        for (int i=0;i<pieceNumber;i++){
            for (int j=0;j<pieceNumber;j++){
                ...
                Piece *pieceDB = [[self pieceOfCurrentPuzzleDB:j+pieceNumber*i] retain];
                
                if (pieceDB!=nil) {
                    
                    PieceView *piece = [[[PieceView alloc] initWithFrame:rect] retain];
                    piece.delegate = self;
                    piece.image = [[UIImage imageWithData:pieceDB.image.data] retain];
        ...
        for (int i=0;i<pieceNumber;i++){
            ...
            for (int j=0;j<pieceNumber;j++){
                ...
                piece.position = -1;
                NSNumber *n = [NSNumber numberWithInt:NumberSquare];
                piece.neighbors = [[[NSArray alloc] initWithObjects:n, n, n, n, nil] retain];
                
                //piece.frame = rect;
                
                
                NSMutableArray *a = [[[NSMutableArray alloc] initWithCapacity:4] retain];
                ...
                if (j>0) {
                    int e = [[[(PieceView *)[arrayPieces lastObject] edges] objectAtIndex:2] intValue];
                ...
                piece.edges = [[NSArray arrayWithArray:a] retain];
    ...
    pieces = [[[NSMutableArray alloc] initWithArray:arrayPieces] retain];
...
- (NSArray *)splitImage:(UIImage *)im partSize:(float)partSize {
    ...
    NSMutableArray *arr = [[[NSMutableArray alloc] initWithCapacity:NumberSquare] retain];
    for (int i=0;i<x;i++){
        for (int j=0;j<y;j++){
...
- (void)updatePieceDB:(PieceView*)piece {
    ...
    //Update piece in the DB
    Piece *pieceDB = [[self pieceOfCurrentPuzzleDB:piece.number] retain];
    pieceDB.position = [[NSNumber numberWithInt:piece.position] retain];
    pieceDB.angle = [[NSNumber numberWithFloat:piece.angle] retain];
    pieceDB.moves = [[NSNumber numberWithInt:piece.rotations] retain];
    pieceDB.rotations = [[NSNumber numberWithInt:piece.moves] retain];
    [pieceDB setisFreeScalar:piece.isFree];
    
    pieceDB.edge0 = [[piece.edges objectAtIndex:0]retain];
    pieceDB.edge1 = [[piece.edges objectAtIndex:1]retain];
    pieceDB.edge2 = [[piece.edges objectAtIndex:2]retain];
    pieceDB.edge3 = [[piece.edges objectAtIndex:3]retain];
...
- (void)createLattice {
    ...
    lattice = [[[Lattice alloc] init] retain];
    ...
    imageViewLattice = [[[UIImageView alloc] initWithImage:image] retain];
//    imageViewLattice = [[UIImageView alloc] init];
//    imageViewLattice.image = image;
...
- (void)resetLatticePositionAndSizeWithDuration:(float)duration {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            CGPoint center = [self.view convertPoint:[(Lattice *)[lattice objectAtIndex:firstPiecePlace] center] fromView:lattice];
...
- (void)organizeDrawerWithOrientation:(UIImageOrientation)orientation {
    ...
    NSMutableArray *temp = [[[NSMutableArray alloc] initWithArray:pieces]retain];
...
- (void)swipeInDirection:(UISwipeGestureRecognizerDirection)direction {
    ...
    if (UIInterfaceOrientationIsLandscape(self.interfaceOrientation)) {
        ...
        PieceView *p = [[temp lastObject] retain];
...
- (BOOL)saveGame {
    ...
    puzzleDB.moves = [[NSNumber numberWithInt:moves] retain];
    puzzleDB.rotations = [[NSNumber numberWithInt:rotations] retain];
    puzzleDB.score = [[NSNumber numberWithInt:score] retain];
    puzzleDB.lastSaved = [[NSDate date]retain];
    puzzleDB.percentage = [[NSNumber numberWithInt:[self completedPercentage]] retain];
...
- (Puzzle*)lastSavedPuzzle {
    ...
    NSFetchRequest *fetchRequest1 = [[[NSFetchRequest alloc] init]retain];
    
    NSEntityDescription *entity = [[NSEntityDescription entityForName:@"Puzzle"  inManagedObjectContext: managedObjectContext] retain];
    
    [fetchRequest1 setEntity:entity];
    
    NSSortDescriptor *dateSort = [[[NSSortDescriptor alloc] initWithKey:@"lastSaved" ascending:NO] retain];
...
- (void)createPuzzleInDB {
    ...
    puzzleDB = [[self newPuzzleInCOntext:managedObjectContext] retain];
    Image *imageDB = [[self newImageInCOntext:managedObjectContext] retain];
    imageDB.data = UIImageJPEGRepresentation(image, 1);
    puzzleDB.image = imageDB;
    puzzleDB.pieceNumber = [[NSNumber numberWithInt:pieceNumber] retain];
    
    for (PieceView *piece in pieces) {
        
        //Creating the piece in the database
        Piece *pieceDB = [[self newPieceInCOntext:managedObjectContext] retain];
        pieceDB.puzzle = puzzleDB;
        pieceDB.number = [[NSNumber numberWithInt:piece.number]retain];
        pieceDB.position = [[NSNumber numberWithInt:piece.position]retain];
        pieceDB.angle = [[NSNumber numberWithFloat:piece.angle]retain];
        pieceDB.angle = [[NSNumber numberWithFloat:piece.angle]retain];
        Image *imagePieceDB = [[self newImageInCOntext:managedObjectContext] retain];
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    [UIView animateWithDuration:duration animations:^{
        [self organizeDrawerWithOrientation:(UIImageOrientation)toInterfaceOrientation];
    }];    
...
- (void)shuffle {
    ...
    for (int i=0; i<NumberSquare; i++) {          
        PieceView *p = [[pieces objectAtIndex:i]retain];
        CGRect rect = p.frame;
...
puzzleDB = [self lastSavedPuzzle];
    ...
    @try {
           ...
           puzzleDB.elapsedTime = [[NSNumber numberWithFloat:elapsedTime] retain];
    }
...
- (void)startTimer {
    ...
    if (!loadingFailed && ![self isPuzzleComplete]) {
        ...
        timer = [[NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(oneSecondElapsed) userInfo:nil repeats:YES] retain];
    }
}
...
- (void)startNewGame {
    ...
    groups = nil;
    pieces = nil;
    groups = [[[NSMutableArray alloc] initWithCapacity:NumberSquare] retain];
    pieces = [[[NSMutableArray alloc] initWithCapacity:NumberSquare] retain];
