...
- (void)dismissPicker {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        ...
        [self.delegate.delegate dismissViewControllerAnimated:YES completion:NULL];
    }   
    
    
    CGRect superRect = self.view.superview.frame;
    self.view.frame = CGRectMake(0, 0, superRect.size.width, superRect.size.height);
}
...
- (IBAction)selectImageFromPuzzleLibrary:(id)sender {
    ...
    IF_IPAD {
        ...
        [self.delegate.delegate presentViewController:c animated:YES completion:NULL];
    }
...
//     [menu startNewGame:nil];
    [delegate.delegate loadPuzzle:[delegate.delegate lastSavedPuzzle]];
...
//    delegate.delegate.image = nil;

//    delegate.delegate.image = [[[UIImage alloc] init] retain];
    delegate.delegate.image = image.image;
    delegate.delegate.imageView.image = delegate.delegate.image;
    delegate.delegate.imageViewLattice = [[[UIImageView alloc] init] retain];
