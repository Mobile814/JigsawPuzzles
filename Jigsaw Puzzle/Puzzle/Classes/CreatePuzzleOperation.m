...
- (void)main {
    ...
    insertionContext = [[[NSManagedObjectContext alloc] init] retain];
    ...
    if (delegate && [delegate respondsToSelector:@selector(puzzlechanged:)]) {
        [[NSNotificationCenter defaultCenter] addObserver:delegate selector:@selector(puzzlechanged:) name:NSManagedObjectContextObjectsDidChangeNotification object:self.insertionContext];
    }
    int pieceNumber = delegate.pieceNumber;
    int NumberSquare = delegate.NumberSquare;
    
    NSLog(@"piecenumber = %d,Numbersquare = %d",pieceNumber,NumberSquare);
    
        
    NSMutableArray *arrayPieces = [[[NSMutableArray alloc] initWithCapacity:NumberSquare] retain];
    ...
    @try {
        ...
        if (!delegate.loadingGame) {
            ...
            DLog(@"Puzzle created in the DB from create");
...
- (NSArray *)splitImage:(UIImage *)im partSize:(float)partSize {
    ...
    NSMutableArray *arr = [[[NSMutableArray alloc] initWithCapacity:delegate.NumberSquare] retain];
    for (int i=0;i<x;i++){
        for (int j=0;j<y;j++){
